...
describe EthereumContractABI::ContractInterface::EIP::ERC721Metadata do
  describe "is_implemented?" do
    it "should return false when contract does not implement" do
      ...
      expect(EIP::ERC721Metadata.is_implemented_by?(contract)).to(eq(false))
    end
    ...
    it "should return true when contract implements erc721 metadata" do
      ...
      expect(EIP::ERC721Metadata.is_implemented_by?(contract)).to(eq(true))
    end
  end
end